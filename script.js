...
const select_box = document.querySelector(".options"),
  ...
  input_country_code = document.querySelector("#country_code"),
  selected_option = document.querySelector(".selected_option div"),
  noResultMessage = document.querySelector(".no-result-message"),
  listElement = select_box.querySelector("ol");

let options = null;

// Groupes de pays (ex: AES)
const countryGroups = {
  "Pays de AES": ["Burkina Faso", "Mali", "Niger"],
  "Autres pays": countries
    .map(c => c.name)
    .filter(name => !["Burkina Faso", "Mali", "Niger"].includes(name)),
};
...
function initializeCountrySelect() {
  ...
  for (const [groupName, groupCountries] of Object.entries(countryGroups)) {
    const groupOptions = [];

    for (const countryName of groupCountries) {
      ...
      option.innerHTML = `
      <div>
        <img src="drapeau_svg/${country.code}.svg" alt="${country.name}" class="flag-icon" />
...
// Initialiser le pays sélectionné par défaut
...
function selectOption() {
  ...
  selected_option.append(flag, phone_code);

  input_country_code.value = phone_code.innerText.replace("+", "00");
...
// Recherche par nom ou indicatif
...
function updateNoResultMessage() {
  if (options.length === 0) {
    noResultMessage.innerText = "Aucun pays disponible";
    noResultMessage.classList.remove("hide");
    return;
  }

  const visibleOptions = options.filter(opt => !opt.classList.contains("hide"));
  if (visibleOptions.length === 0) {
    noResultMessage.innerText = "Aucun pays ne correspond à la recherche";
    noResultMessage.classList.remove("hide");
  } else {
    noResultMessage.classList.add("hide");
  }

  // Masquer les groupes vides après filtrage
